        ...
        (function() {
            ...
            if (!el) return;
            const strings = (window.__TYPED_ROLES__ && window.__TYPED_ROLES__.length)
                ? window.__TYPED_ROLES__
                : ['Software Engineer', 'C++ Developer', 'Full Stack Developer', 'Systems Thinker'];
            let i = 0,
                j = 0,
                isDeleting = false,
                currentText = '';
        ...
        (function() {
            ...
            async function fetchGitHubData() {
                ...
                try {
                    ...
                    // Fetch user profile and repos directly from GitHub API
                    const [userRes, reposRes] = await Promise.all([
                        fetch(`https://api.github.com/users/${GITHUB_USERNAME}`, { signal: controller.signal }),
                        fetch(`https://api.github.com/users/${GITHUB_USERNAME}/repos?sort=stars&per_page=12`, { signal: controller.signal })
                    ]);
                    ...
                    const allData = { user, repos, totalStars, totalRepos: user.public_repos };
        ...
        // ═══════════════════════════════════════════════════════════════
        // PROJECTS — served from hardcoded data (backend removed)
        // ═══════════════════════════════════════════════════════════════
        (function() {
            // Projects are rendered directly in index.html as hardcoded cards.
            // No backend API needed. The hardcoded HTML remains visible as-is.
        })();

        // ═══════════════════════════════════════════════════════════════
        // CONTACT FORM — submits directly to Web3Forms
        // ═══════════════════════════════════════════════════════════════
        (function() {
            ...
            form.addEventListener('submit', async function(e) {
                ...
                try {
                    const response = await fetch('https://api.web3forms.com/submit', {
                        method: 'POST',
                        body: JSON.stringify({ 
                            access_key: 'c8ef8644-e6d8-4499-8258-a01ff4daa1f1',
                            name, 
                            email, 
                            message 
                        }),
